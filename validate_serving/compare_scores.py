...
def get_join_rates(output_path):
    ...
    query = f"""SELECT 1 - COUNTIF(score_df is NULL) / COUNT(score) AS bq_join_rate
                FROM {project_id}.{dataset_id}.bq_df_scores"""
    bq_join_rate = t.query(query)["bq_join_rate"][0]
    query = f"""SELECT 1 - COUNTIF(score is NULL) / COUNT(score_df) AS df_join_rate
                FROM {project_id}.{dataset_id}.bq_df_scores"""
    df_join_rate = t.query(query)["df_join_rate"][0]

    # write to file
    with open(output_path, "w") as f:
        f.write(f"BQ join rate: {bq_join_rate}\n")
        f.write(f"DF join rate: {df_join_rate}")
...
def feature_mismatch_rate(feature, tolerance=0.01):
    ...
    SELECT 
        '{feature}' AS feature,
        COUNTIF(ABS({feature} - {feature}_df) > {tolerance}) / COUNT(1) AS mismatch_rate
    FROM `{project_id}.{dataset_id}.bq_df_scores`
    """
    df = t.query(query)
    return df
...
def score_difference_distribution(output_path):
    ...
    WITH quantile_array AS (
        SELECT APPROX_QUANTILES(ABS(score - score_df), 100) AS quantile
        FROM `fraud_detection.bq_df_scores`
    )
    SELECT quantile
